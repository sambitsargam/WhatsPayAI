...
async def process_ai_query(message: str) -> Tuple[str, int, int]:
    ...
    try:
        ...
        logger.info(f"AI query processed: {input_tokens} input + {output_tokens} output tokens")
...
async def handle_ai_query(user_id: str, message: str) -> str:
    ...
    try:
        ...
        estimated_output_tokens = min(MAX_TOKENS, estimated_input_tokens)  # Conservative estimate
        ...
        response += f"💰 Cost: {actual_cost:.4f} HTR | Balance: {balances[user_id]:.4f} HTR"
