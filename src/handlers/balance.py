...
async def handle_balance_query(user_id: str) -> str:
    ...
    try:
        ...
        if current_balance <= 0:
            ...
            estimated_queries = (
                int(current_balance / 0.01) * 100
            )  # Assuming 0.01 HTR per 100 tokens
...
async def handle_usage_history(user_id: str, limit: int = 10) -> str:
    ...
    try:
        ...
        sorted_usage = sorted(
            user_usage, key=lambda x: x.get("timestamp", "1970-01-01"), reverse=True
        )

        message = (
            f"📋 **Usage History** (Last {min(limit, len(sorted_usage))} entries)\n\n"
        )
