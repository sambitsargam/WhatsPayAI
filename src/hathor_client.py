...
class HathorClient:
    ...
    def __init__(self, node_url: str = HATHOR_NODE_URL):
        ...
        self.session.headers.update(
            {"Content-Type": "application/json", "User-Agent": "WhatsPayAI/1.0"}
        )
    ...
    def get_address_info(self, address: str) -> Optional[dict]:
        ...
        return self._make_request(
            f"thin_wallet/address_info", "GET", {"address": address}
        )

    def get_address_history(self, address: str) -> Optional[dict]:
        """Get transaction history for an address."""
        return self._make_request(
            f"thin_wallet/address_history", "GET", {"address": address}
        )
