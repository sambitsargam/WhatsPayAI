...
class TestWebhookIntegration:
    ...
    def mock_all_external_services(self):
        ...
        with patch("src.twilio_client.send_whatsapp_message") as mock_twilio, patch(
            "src.intent.classify_with_openai"
        ) as mock_openai_intent, patch(
            "src.handlers.ai_query.openai.ChatCompletion.create"
        ) as mock_openai_chat, patch(
            "src.hathor_client.get_user_address"
        ) as mock_address:
            # Configure mocks
    ...
    def test_webhook_payment_intent(
        self, client, mock_all_external_services, clear_app_state
    ):
        """Test webhook with payment intent."""
        response = client.post(
            "/webhook", data={"From": "whatsapp:+1234567890", "Body": "top up 1 HTR"}
        )
    ...
    def test_webhook_balance_intent(
        self, client, mock_all_external_services, clear_app_state
    ):
        """Test webhook with balance intent."""
        response = client.post(
            "/webhook",
            data={"From": "whatsapp:+1234567890", "Body": "what's my balance?"},
        )
    ...
    def test_webhook_help_intent(
        self, client, mock_all_external_services, clear_app_state
    ):
        """Test webhook with help intent."""
        response = client.post(
            "/webhook", data={"From": "whatsapp:+1234567890", "Body": "help me please"}
        )
    ...
    def test_webhook_ai_query_insufficient_balance(
        self, client, mock_all_external_services, clear_app_state
    ):
    ...
    def test_webhook_ai_query_with_balance(
        self, client, mock_all_external_services, clear_app_state
    ):
    ...
    def test_webhook_empty_message(
        ...
        response = client.post(
            "/webhook", data={"From": "whatsapp:+1234567890", "Body": ""}
        )
